...
@keyframes borderGlow {
  ...
  50%     { box-shadow: 0 0 30px var(--green-glow), 0 0 60px rgba(0,100,200,0.06), 0 0 100px rgba(0,232,122,0.03), inset 0 0 20px rgba(0,0,0,0.3); }
}
...
/* ── END SCREEN ── */
  ...
  position: absolute;
  top: 0; left: 0; right: 0; height: 2px;
  background: linear-gradient(to right, transparent, var(--green) 30%, var(--cyan) 70%, transparent 100%);
  opacity: 0.6;
}
 ...
 * ```
 */
export default async function initHeistGame(options = {}) {
  ...
  if (!environment.gameContainer) {
    throw new Error('Heist game requires a valid gameContainer element.');
  }

  // Inject CSS
  injectHeistCSS();

  if (!environment.gameCanvas) {
    console.warn('No gameCanvas found in the page; HeistLevel will create one automatically.');
  }
